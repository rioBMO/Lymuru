...
export function AudioConverterPage() {
    ...
    const [outputFormat, setOutputFormat] = useState<"mp3" | "m4a" | "wav" | "aiff" | "opus">("mp3");
    const [bitrate, setBitrate] = useState("320k");
    const [m4aCodec, setM4aCodec] = useState<"aac" | "alac">("aac");
                ...
                {files.length === 0 ? (
                    <>
                        ...
                        <p className="text-sm text-muted-foreground mb-4 text-center">
                            Select audio files or a folder to convert
                        </p>
                    ...
                    <div className="w-full h-full p-6 space-y-4 flex flex-col">
                            ...
                            <div className="flex items-center gap-4 flex-wrap">
                                <div className="flex items-center gap-2">
                                    ...
                                    <ToggleGroup
                                        ...
                                        value={outputFormat}
                                        onValueChange={(value) => {
                                            if (value) setOutputFormat(value as typeof outputFormat);
                                        }}
                                    >
                                        {!isFormatDisabled && (
                                            <ToggleGroupItem value="mp3" aria-label="MP3">MP3</ToggleGroupItem>
                                        )}
                                        <ToggleGroupItem value="m4a" aria-label="M4A">M4A</ToggleGroupItem>
                                        <ToggleGroupItem value="opus" aria-label="Opus">Opus</ToggleGroupItem>
                                        <ToggleGroupItem value="wav" aria-label="WAV">WAV</ToggleGroupItem>
                                        <ToggleGroupItem value="aiff" aria-label="AIFF">AIFF</ToggleGroupItem>
                                    </ToggleGroup>
                                </div>
                                ...
                                {outputFormat === "m4a" && hasFlacFiles && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <ToggleGroup
                                            ...
                                            value={m4aCodec}
                                            onValueChange={(value) => {
                                                if (value) setM4aCodec(value as typeof m4aCodec);
                                            }}
                                        >
                        ...
                        <div className="flex items-center justify-between shrink-0">
                            <div className="text-sm text-muted-foreground">
                                {files.length} file(s) &bull; {successCount} converted
                            </div>
                        </div>
                            ...
                            {files.map((file) => (
                                <div key={file.path} className="flex items-center gap-3 rounded-lg border p-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {file.error && (
                                            <p className="truncate text-xs text-destructive">{file.error}</p>
                                        )}
                                    </div>
